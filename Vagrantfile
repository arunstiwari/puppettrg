...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "puppetserver", autostart: false do |puppetserver|
    ...
    # PuppetDB Server
    config.vm.define "puppetdb", autostart: false do |puppetdb|
      puppetdb.vm.hostname = "puppetdb.example.com"
      puppetdb.vm.network :private_network, ip: "192.168.250.8"
      puppetdb.vm.provision "shell", inline: $setupscript
    end
